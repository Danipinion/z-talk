    ...
    init {
        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            database.friendDao().getAllFriends().collect { localFriends ->
                val mapped = localFriends.map { FriendResponse(it.id, it.username, it.avatar, it.mood, it.background) }
        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            database.friendRequestDao().getAllFriendRequests().collect { localRequests ->
                val mapped = localRequests.map { FriendRequestResponse(it.senderId, it.senderUsername, it.status, it.createdAt, it.senderAvatar, it.senderMood, it.senderBackground) }
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
                        ...
                        database.friendRequestDao().insertFriendRequests(remoteRequests.map {
                            FriendRequestEntity(it.senderId, it.senderUsername, it.status, it.createdAt, it.senderAvatar, it.senderMood, it.senderBackground)
                        })
                    }
                } else {
                    if (_requestsState.value !is AuthState.Success) {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
                        ...
                        database.friendDao().insertFriends(remoteFriends.map {
                            FriendEntity(it.id, it.username, it.avatar, it.mood, it.background)
                        })
                    }
                } else {
                    if (_friendsState.value !is AuthState.Success) {
